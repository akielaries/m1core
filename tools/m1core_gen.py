...
def gen_sim_header(mcu, types, source):
    """the clock, for testbenches

    tb_core reports MIPS, which needs the clock rate, and it had 25 MHz written
    into it as a literal long after the design moved to 45. a number that only
    appears in a printf still misleads whoever reads it
    """
    return (f"// generated by tools/m1core_gen.py from {source}, do not edit\n"
            f"`define SYSTEM_CLOCK_HZ {mcu['clock']['hz']}\n")
...
def main():
    ...
    ap.add_argument("--sim-header", help="clock define for the testbenches")
    ...
    if args.sim_header:
        outputs.append((args.sim_header, gen_sim_header(mcu, types, src)))
    if args.mcu_rtl:
