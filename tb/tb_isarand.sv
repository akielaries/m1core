...
// runs the self checking randomised isa test on the core, no debugger involved
//
...
//   0x20000000 error count, 0x20000004 first failing block,
//   0x20000008 completion marker

`include "m1core_clock.vh"

`include "isarand_blocks.vh"
...
module tb_isarand;
  ...
  m1core_mcu #(
    ...
    .DTCM_WORDS (2048),
    .GPIO_WIDTH (2),
    .ITCM_INIT  ("../sw/baremetal/tests/isarand/build/isarand.hex")
  ) dut (
    .clk   (clk),
    .rst_n (rst_n),
    .swclk (1'b0),
  ...
  initial begin
    if ($test$plusargs("vcd")) begin
      $dumpfile("tb_isarand.vcd");
      $dumpvars(0, tb_isarand);
    end
    ...
    if (r_done !== DONE_MAGIC) begin
      $display("FAIL randomised isa test never completed, pc=%08x state=%0d after %0d cycles",
               dut.u_core.pc, dut.u_core.state, cycles);
      $display("FAIL");
    end else if (r_errors !== 32'd0) begin
      $display("FAIL %0d word(s) differ, first in block %0d",
               r_errors, r_firstid - 1);
      $display("     word %0d (%s): core %08x, model %08x",
               dut.u_dtcm.mem[3], (dut.u_dtcm.mem[3] == 0) ? "apsr" : "reg",
               dut.u_dtcm.mem[4], dut.u_dtcm.mem[5]);
      $display("FAIL");
    end else begin
      $display("ok   randomised isa test completed in %0d cycles", cycles);
      $display("ok   all instruction checks passed");
      $display("     %0d blocks, model and rtl agree on every register and flag",
               `ISARAND_BLOCKS);
      $display("");
      $display("PASS");
    end
