// board layer for the tang mega 60K (GW5AT-LV60PG484AC2/I1)
//
...
module top (
  input        HCLK,          // 50 MHz system clock, ball V22
  input        hwRstn,        // reset, ACTIVE LOW on this board, ball AA13
  ...
  // boards/mega60k/src/gowin_pll/, and list only that copy in this board's
  ...
  // the wrapper's port list depends on the device AND on what you tick when
  // generating, so this cannot be copied between boards. tools/check_pll.py
  // compares it against the wrapper on disk. mdclk must be a real running
  // clock: the wrapper feeds it to PLL_INIT with CLK_PERIOD = 20
  Gowin_PLL u_pll (
    .clkin   (HCLK),
    .clkout0 (pll_clk0),
    .lock    (pll_lock),
